...
class CacheConfiguration(
    ...
    fun hazelcastInstance(jHipsterProperties: JHipsterProperties): HazelcastInstance {
        ...
        val hazelCastInstance = Hazelcast.getHazelcastInstanceByName("kinvoice")
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized")
            return hazelCastInstance
        }
        val config = Config()
        config.instanceName = "kinvoice"
        ...
        config.mapConfigs["com.sendilkumarn.sample.kinvoice.domain.*"] = initializeDomainMapConfig(jHipsterProperties)
